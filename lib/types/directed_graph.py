...
class DirectedGraph:
    ...
    def dijkstraWaypoint(self, start, W1, W2, avoid=None):
        """Find the shortest path between two nodes using Dijkstra's algorithm, avoiding specified nodes.
        ...
        if len(avoid > 0):
            ...
            try:
                # Use Dijkstra's algorithm to find the shortest path
                path1 = nx.shortest_path(G, source=start, target=W1, weight='weight', method='dijkstra')
                path2 = nx.shortest_path(G, source=W1, target=W2, weight='weight', method='dijkstra')
                return path1 + path2
            except nx.NetworkXNoPath:
                return None
        else:
            try:
                # Use Dijkstra's algorithm to find the shortest path
                path1 = nx.shortest_path(self.graph, source=start, target=W1, weight='weight', method='dijkstra')
                path2 = nx.shortest_path(self.graph, source=W1, target=W2, weight='weight', method='dijkstra')
                return path1 + path2
            except nx.NetworkXNoPath:
                return None